...
int txtValue = 0, thresHold = 150, signalDetectionValue=1000;
...
void loop()
{
  ...
  if (pulseIn(sensorSignal, HIGH) > signalDetectionValue)
    if (pulseIn(sensorSignal, HIGH) <thresHold)
    {
      //delay(1);
        for (int i = 0; i < 8; i++)if (pulseIn(sensorSignal, HIGH) > thresHold) senzorA += "1"; else senzorA += 0; //signal[i]=pulseIn(sensorSignal,HIGH);
        for (int i = 0; i < 8; i++)if (pulseIn(sensorSignal, HIGH) > thresHold) senzorB += "1"; else senzorB += 0; //signal[i]=pulseIn(sensorSignal,HIGH);
        for (int i = 0; i < 8; i++)if (pulseIn(sensorSignal, HIGH) > thresHold) senzorC += "1"; else senzorC += 0; //signal[i]=pulseIn(sensorSignal,HIGH);
        for (int i = 0; i < 8; i++)if (pulseIn(sensorSignal, HIGH) > thresHold) senzorD += "1"; else senzorD += 0; //signal[i]=pulseIn(sensorSignal,HIGH);
